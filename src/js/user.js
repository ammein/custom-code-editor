...
apos.define('custom-code-editor', {
    ...
    construct: function (self, options) {
            ...
            switch (type) {
                case 'slider':
                    (function () {
                        ...
                        let forLabel = apos.utils.cssName(object.name).replace(/-/g, ' ');
                        let category = apos.utils.cssName(object.category).replace(/-/g, ' ');
                ...
                case 'dropdownArray':
                    (function () {
                        let label = document.createElement('label');
                        let forLabel = apos.utils.cssName(object.name).replace('-', ' ');
                        let category = apos.utils.cssName(object.category).replace(/-/g, ' ');
                        label.innerHTML = forLabel + ' :';
                        label.setAttribute('for', object.name);
                        $(label).css('text-transform', 'capitalize');
        ...
        self.populate = function (object, name, $field, $el, field, callback) {
            ...
            if (self.has(self.ace, 'config.dropdown.enable')) {
                ...
                for (let i = 0; i < self.ace.modes.length; i++) {
                    let dropdown = $fieldSet.find('.dropdown-content');
                ...
                let allItems = $fieldSet.find('.dropdown-content > li');
            ...
            let optionsContainer = $fieldSet.find('.options-container');
            let optionsInner = $fieldSet.find('.options-inner');
            let unorderedLists = document.createElement('ul');
            let buttonOptions = $fieldSet.find('.button-options');
            let moreOptions = $fieldSet.find('.press-more-options');
            let optionsCustomizer = self.has(self.ace.config, 'optionsCustomizer') ? self.ace.config.optionsCustomizer.enable : true;
                ...
                function loopOptions(myOptions) {
                    ...
                    for (let key of Object.keys(editor.getOptions())) {
                        if (editor.getOptions().hasOwnProperty(key)) {
                            ...
                            let optionsTypes = self.ace.optionsTypes;
                    ...
                    // Add Category
                    let lists = unorderedLists.childElementCount;
                    let inc = 0;
                    for (let i = 0; i < lists; i++) {
                        let h1 = document.createElement('h1');
                        let li = document.createElement('li');
                        $(h1).addClass('category');
                        let category = unorderedLists.children[inc].dataset.category;
                    ...
                    optionsInner.find('[data-header]').on('click', function () {
                        ...
                        let nextSiblings = $(header).nextUntil(`li:not([data-category="${header.dataset.category}"])`);
                    ...
                    $($fieldSet.find('.my-options')).on('keyup', function () {
                        let input, filter, ul, myLists, i, header, txtValue;
        ...
        self.convert = function (data, name, $field, $el, field, callback) {
            ...
            // Get Editor
            let editor = $fieldSet.data('editor');
